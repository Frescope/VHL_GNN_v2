...
import h5py
import pickle
from Test_trans2_autothresh2_transformer import transformer
...
def evaluation_autothresh_valid(concept_lists, summary_lists, query_summary, Tags, test_vids, concepts, queries):
    # 在验证集上生成门限表
    # 首先根据两组concept_logits选出一组候选集，然后从候选里根据summary_logits做最终预测
    c_logits = concept_lists[0]  # (bc*seq_len) * 48
    s_logits = summary_lists[0]  # (bc*seq_len) * 45
    ...
    for i in range(len(test_vids)):
        ...
        s_predictions = s_logits[pos: pos + vlength]
        video_preds[vid] = {}
        video_preds[vid]['concept'] = c_predictions
        video_preds[vid]['summary'] = s_predictions
    # 基于concept的门限表
    threshold_table = {}
    for c in concepts:
        threshold_table[c] = []
    ...
    for i in range(len(test_vids)):
        vid, vlength = test_vids[i]
        summary = query_summary[str(vid)]
        c_preds = video_preds[vid]['concept']
        s_preds = video_preds[vid]['summary']
        for query in summary:
            shots_gt = summary[query]
            q_ind = queries[vid].index(query)
            c1, c2 = query.split('_')
            c1_ind = concepts.index(c1)
            c2_ind = concepts.index(c2)
            c1_logits = c_preds[:, c1_ind]
            c2_logits = c_preds[:, c2_ind]
            q_logits = s_preds[:, q_ind]
            scores = (c1_logits + c2_logits) / 2 * ratio + q_logits * (1 - ratio)
            # 按列做标准化
            ...
            scores_indexes = scores[candidate].reshape((-1, 1))
            scores_indexes = np.hstack((scores_indexes, candidate))  # N*2，候选集及其原索引
            scores_indexes = scores_indexes[scores_indexes[:, 0].argsort()]  # 对预测排序

            # 测试这一最终预测在不同门限下的表现，取最优结果登记
            max_p = 0
            max_r = 0
            max_f1 = 0
            max_thresh = 0
            for step in range(16, 25, 2):
                rank_ratio = step / 1000  # 预测长度占比
                hl_num_s2 = math.ceil(vlength * rank_ratio)  # stage 2
                threshold = scores_indexes[-hl_num_s2, 0]
                shots_pred = scores_indexes[-hl_num_s2:, 1].astype(int)
                shots_pred.sort()
                # compute
                sim_mat = similarity_compute(Tags, int(vid), shots_pred, shots_gt)
                weight = shot_matching(sim_mat)
                ...
                if f1 > max_f1:
                    ...
                    max_f1 = f1
                    max_thresh = threshold
            threshold_table[c1].append(max_thresh)
            threshold_table[c2].append(max_thresh)
            PRE_values.append(max_p)
            REC_values.append(max_r)
            F1_values.append(max_f1)
    PRE_value = np.array(PRE_values).mean()
    REC_value = np.array(REC_values).mean()
    F1_value = np.array(F1_values).mean()

    thresh_mean = 0  # 全局的平均门限
    zero_count = 0  # 在当前视频中没有出现过的concept
    for c in threshold_table:
        if len(threshold_table[c]) == 0:
            threshold_table[c] = None
            zero_count += 1
            continue
        threshold_table[c] = np.array(threshold_table[c]).mean()
        thresh_mean += threshold_table[c]
    thresh_mean /= len(threshold_table) - zero_count
    for c in threshold_table:
        if threshold_table[c] == None:
            threshold_table[c] = thresh_mean

    return PRE_value, REC_value, F1_value, threshold_table
    ...
    with tf.Graph().as_default():
        ...
        for step in range(hp.maxstep):
            ...
            if step % epoch_step == 0 or (step + 1) == hp.maxstep:
                ...
                p, r, f, table = evaluation_autothresh_valid(concept_lists, summary_lists, query_summary, Tags, test_vids, concepts, queries)
                ...
                if step > MIN_TRAIN_STEPS - PRESTEPS and f >= max_f1:
                    ...
                    table_path = model_path + '_table.json'
                    with open(table_path, 'w') as file:
                        json.dump(table, file, cls=NpEncoder)

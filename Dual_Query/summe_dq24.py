# 与summe_dq22相同，使用transformer_dq24
...
class Path:
    ...
    # loss参数
    parser.add_argument('--loss_pred_ratio', default=1.00, type=float)  # pred损失比例
...
def load_data(uniset_base, hp):
    ...
    for vid in uniset_labels[name]:
        ...
        # testing labels
        multi_scores = np.array(uniset_labels[name][vid]['multi_score'])
        shot_scores = frame2shot(multi_scores, hp)
        shot_binary = []
...
def main(self):
    ...
    data, uniset_labels, queries, query_embedding = load_data(UNISET_BASE, hp)
    ...
    for kfold in range(kfold_start, kfold_end + 1):
        ...
        for i in range(hp.repeat):
            ...
            logging.info('*' * 60)
            running(data_train, data_valid, uniset_labels, queries, query_embedding, 0, model_save_dir, '')
            logging.info('*' * 60)

...
def segment_embedding(inputs, hp, scope='segment_embedding'):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        clip_nodes, segment_nodes, memory_nodes = tf.split(inputs, [hp.seq_len, hp.segment_num, hp.memory_num], 1)  # 沿seq_len切开

        clip_nodes += clip_embs
        segment_nodes += segment_embs
        memory_nodes += memory_embs

        outputs = tf.concat([clip_nodes, segment_nodes, memory_nodes], axis=1)
        return outputs
...
def transformer(segment_embs, features, memory_embs,
                segment_positions, feature_positions, scores_src, drop_out, training, hp, concept_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        input_nodes = tf.concat([visual_nodes, segment_nodes, memory_nodes], axis=1)
        ...
        sigmoid_score = tf.sigmoid(prediction_logits)
        softmax_logits = tf.nn.softmax(prediction_logits, axis=1)
        aux_score = tf.nn.softmax(tf.reduce_sum(softmax_logits, axis=2, keepdims=True), axis=1)  # 附加得分，对应到每个片段

        return shots_output, memory_output, sigmoid_score, aux_score
...
def load_feature_4fold(feature_base, s1_label_path, Tags):
    ...
    for vid in range(1,5):
        ...
        feature = np.load(feature_path).reshape((SHOTS_NUMS[vid - 1], FRAME_PER_SHOT, D_VISUAL))  # 按照5帧为一个shot分组
...
def segment_embedding_build(data, hp):
    ...
    for vid in data:
        ...
        for i in range(segnum):
            ...
            poses.append(mid)
            embs = data[vid]['feature'][start : end].reshape(((end-start) * FRAME_PER_SHOT, D_VISUAL))
            if mode == 'mean':
...
def get_batch_train(data_train, segment_dict, train_scheme, concepts, concept_embedding, step, hp):
    ...
    positions = np.reshape(positions, (batch_num, hp.seq_len * FRAME_PER_SHOT))
...
def get_batch_test(data_test, segment_dict, test_scheme, concepts, concept_embedding, step, hp):
    ...
    for i in range(batch_num):
        ...
        if padding_len > 0:
            ...
            position_pad = np.array([vlength] * padding_len * FRAME_PER_SHOT)
            score_pad = np.zeros(padding_len * FRAME_PER_SHOT)
...
def run_training(data_train, data_test, query_summary, Tags, concepts, concept_embedding, segment_dict,
    ...
    with tf.Graph().as_default():
        ...
        # placeholders
        features_holder = tf.placeholder(tf.float32, shape=(hp.bc * hp.gpu_num, hp.seq_len * FRAME_PER_SHOT, D_VISUAL))
        positions_holder = tf.placeholder(tf.int32, shape=(hp.bc * hp.gpu_num, hp.seq_len * FRAME_PER_SHOT))
        ...
        for step in range(hp.maxstep):
            ...
            if step % epoch_step == 0 or (step + 1) == hp.maxstep or test_mode == 1:
                ...
                if test_mode == 0 and (step + 1) != hp.maxstep and (step < hp.protection or not int(epoch) % hp.eval_epoch == 0):
                    continue  # 增大测试间隔
                ...
                for test_step in range(max_test_step):
                    ...
                    for preds in temp_list:
                        pred_s1_lists.append(preds.reshape((-1, hp.query_num)))

...
def segment_embedding(inputs, hp, scope='segment_embedding'):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        visual_nodes, segment_nodes, memory_nodes = inputs

        visual_nodes += visual_embs
        segment_nodes += segment_embs
        memory_nodes += memory_embs

        outputs = tf.concat([visual_nodes, segment_nodes, memory_nodes], axis=1)
        return outputs
    ...
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        if hp.local_attention_pose == 'early':
            ...
            scores_src_shots, scores_src_rem = tf.split(scores_src,
                                                        [hp.seq_len*FNUM, hp.segment_num+hp.memory_num],
                                                        axis=1)
        ...
        input_nodes = [visual_nodes, segment_nodes, memory_nodes]
        ...
        visual_branch = tf.layers.dense(visual_branch, hp.query_num, use_bias=True, activation=None)

        sigmoid_score = tf.sigmoid(visual_branch)  # 对每个query，计算所有片段的相关性，片段间不影响

        return shot_output, memory_output, sigmoid_score

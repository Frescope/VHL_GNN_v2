...
def segment_embedding(inputs, hp, scope='segment_embedding'):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        visual_nodes, segment_nodes, query_nodes = inputs

        visual_nodes += visual_embs
        segment_nodes += segment_embs
        query_nodes += query_embs

        outputs = tf.concat([visual_nodes, segment_nodes, query_nodes], axis=1)
        return outputs
...
def transformer(img_embs, segment_embs, txt_embs,
    ...
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        if hp.local_attention_pose == 'early':
            ...
            scores_src_shots, scores_src_rem = tf.split(scores_src,
                                                        [hp.seq_len*FNUM, hp.segment_num+hp.query_num],
                                                        axis=1)
        ...
        input_nodes = [visual_nodes, segment_nodes, query_nodes]
        ...
        query_output = enc_output[:, -hp.query_num : , :]
        ...
        return shot_output, sigmoid_score, aux_score

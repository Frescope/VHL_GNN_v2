...
def transformer(img_embs, segment_embs, txt_embs, memory_embs,
    ...
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        frame_output= enc_output[: , 0 : hp.seq_len * 5, :]
        query_output = enc_output[:, -(hp.memory_num+hp.query_num) : -hp.memory_num, :]
        memory_output = enc_output[:, -hp.memory_num : ,]

        # visual branch
        visual_branch = tf.layers.dense(frame_output, 1024, use_bias=True, activation=tf.nn.relu)
        visual_branch = tf.layers.dense(visual_branch, 512, use_bias=True, activation=tf.nn.relu)
        visual_branch = tf.layers.dense(visual_branch, 256, use_bias=True, activation=None)  # bc*(seqlen*5)*D
        visual_branch = tf.reshape(visual_branch, (-1, hp.seq_len, 5, 256))
        visual_branch = tf.reduce_mean(visual_branch, axis=2)  # 平均池化每个shot内部的几帧
        ...
        return frame_output, memory_output, sigmoid_score, aux_score
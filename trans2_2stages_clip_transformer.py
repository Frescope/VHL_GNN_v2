...
def segment_embedding(inputs, hp, scope='segment_embedding'):
    ...
    clip_len = hp.seq_len * 5 + 1  # 多一个global节点
...
def transformer(features, positions, scores_src, concept_emb, global_emb, drop_out, training, hp, c_num, s_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        # encoder & decoder inputs
        concept_nodes = tf.layers.dense(concept_emb, D_MODEL, use_bias=True, activation=None)
        ...
        input_nodes = tf.concat([global_nodes, visual_nodes, concept_nodes], axis=1)
        ...
        decoder_output = memory[: , 1 : hp.seq_len * 5 + 1, :]
        ...
        concept_logits = tf.sigmoid(concept_logits)

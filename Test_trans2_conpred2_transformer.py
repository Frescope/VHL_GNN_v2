...
def transformer(features, positions, scores_src, txt_emb, img_emb, drop_out, training, hp, c_num, s_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        concept_preds = tf.layers.dense(concept_preds, 1, use_bias=True, activation=None)
        concept_preds = tf.squeeze(concept_preds)
        concept_preds = tf.nn.softmax(concept_preds, axis=1)  # 每个序列对应的concept节点输出一个一维向量

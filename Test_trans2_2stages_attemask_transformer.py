...
def multihead_attention(queries, keys, values, key_masks, mask_meta,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        # Attention
        outputs = scaled_dot_product_attention(Q_, K_, V_, key_masks, mask_meta, causality, dropout_rate, training)
...
def transformer(features, positions, scores_src, txt_emb, img_emb, drop_out, training, hp, c_num, s_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        # encoding & decoding
        memory = encoder(input_nodes, src_masks, mask_meta, drop_out, training, hp)

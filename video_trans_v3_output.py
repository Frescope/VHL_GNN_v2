# 基于trans3_check，加载模型，输出各个branch的结果并记录
import os
import time
import numpy as np
...
def prediction_output(concept_lists, summary_lists, pred_s1_lists, test_vids):
    ...
    for i in range(len(test_vids)):
        ...
        outputs[vid] = {}
        outputs[vid]['c_predictions'] = c_logits[pos : pos + vlength]
        outputs[vid]['s_predictions'] = s_logits[pos : pos + vlength]
        outputs[vid]['p_predictions'] = p_logits[pos : pos + vlength]
        pos += vlength
    return outputs
    ...
    with tf.Graph().as_default():
        ...
        for step in range(hp.maxstep):
            ...
            if step % epoch_step == 0 or (step + 1) == hp.maxstep:
                ...
                output = prediction_output(concept_lists, summary_lists, pred_s1_lists, test_vids)
                return output
    return None
...
def main(self):
    ...
    for kfold in range(4):
        ...
        # repeat
        test_outputs[str((kfold + 3) % 4 + 1)] = {}
        model_count = 0
        for r in range(hp.repeat):
            model_save_dir = MODEL_SAVE_BASE + hp.msd + '_%d_%d/' % (kfold, r)
            ...
            for i in range(len(models_to_restore)):
                ...
                output_vid = run_testing(data_train, data_test, queries, query_summary, Tags, concepts, concept_embedding,
                                 model_path)
                test_outputs[str((kfold + 3) % 4 + 1)][model_count] = output_vid[str((kfold + 3) % 4 + 1)]
    with open(MODEL_SAVE_BASE + hp.msd + '_test_outputs.json', 'w') as file:
        json.dump(test_outputs, file, cls=NpEncoder)
    logging.info('Predictions Outputed!')

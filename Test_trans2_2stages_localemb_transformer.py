...
def segment_embedding(inputs, hp, scope='segment_embedding'):
    # local_embs, visual, concepts没类节点对应一个不同的嵌入
    concept_len = inputs.get_shape().as_list()[1] - hp.local_len - hp.seq_len
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        local_emb = tf.get_variable(name='local_posemb', shape=[D_MODEL])
        local_emb = tf.expand_dims(tf.expand_dims(local_emb, 0), 0)  # 1*1*D_model
        ...
        local_nodes, clip_nodes, concept_nodes = tf.split(inputs, [hp.local_len, hp.seq_len, concept_len], 1)  # 沿seq_len切开

        local_nodes += local_emb
        clip_nodes += clip_embs
        concept_nodes += concept_embs

        outputs = tf.concat([local_nodes, clip_nodes, concept_nodes], axis=1)
    return outputs
...
def transformer(local_embs, features, img_emb, positions, scores_src, drop_out, training, hp, c_num, s_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        # encoder & decoder inputs
        local_nodes = tf.layers.dense(local_embs, D_MODEL, use_bias=True, activation=None)  # bc*local_len*D
        ...
        input_nodes = tf.concat([local_nodes, visual_nodes, image_nodes], axis=1)
        ...
        decoder_output = memory[: , hp.local_len : hp.local_len + hp.seq_len, :]  # 只取视频序列部分
        reconst_vecs = decoder_output
        ...
        concept_logits = tf.sigmoid(concept_logits)  # 归一化
        ...
        return concept_logits, summary_logits, reconst_vecs
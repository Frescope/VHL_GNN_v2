...
import scipy.io
import pickle
from trans2_attention_observe.Test_attention_output_transformer import transformer
...
if hp.server == 0:
    ...
    CKPT_MODEL_PATH = r'/data/linkang/model_HL_v4/attention_0_0/S1560-E156-L3.140443-F0.530'
...
def test_scheme_build_shuffle(data_test, seq_len):
    # 打乱输入序列的顺序后再选择clip，输入序列内保持有序（和训练时一样），label & position也要对应操作，输出后根据scheme中的原始记录映射回原顺序
    test_scheme = []
    test_vids = []
    for vid in data_test:
        vlength = len(data_test[str(vid)]['label'])
        seq_num = math.ceil(vlength / seq_len)
        shuffle_index = list(range(vlength))
        random.shuffle(shuffle_index)
        for i in range(seq_num):
            seq_start = i * seq_len
            seq_end = min(vlength, (i + 1) * seq_len)
            clip_list = shuffle_index[seq_start : seq_end]
            clip_list.sort()
            test_scheme.append((vid, clip_list))
        test_vids.append((vid, vlength))
    return test_scheme, test_vids

def get_batch_test_shuffle(data_test, test_scheme, step, gpu_num, bc, seq_len):
    features = []
    labels = []
    positions = []
    scores = []
    ...
    for i in range(batch_num):
        ...
        vid, clip_list = test_scheme[pos]
        vlength = len(data_test[str(vid)]['label'])
        padding_len = seq_len - len(clip_list)
        feature = data_test[str(vid)]['feature'][clip_list, 3, :]  # 最中间的一个特征是无噪声（偏移）的
        label = data_test[str(vid)]['label'][clip_list]
        position = np.array(clip_list)
...
def reverse_map(index):
    x = np.array(index).reshape((-1, 1))
    r = np.array(range(len(x))).reshape((-1, 1))
    x = np.hstack((x, r))
    x = x[x[:, 0].argsort()]
    r = x[:, 1].reshape((-1))
    return list(r)

def evaluation(pred_scores, query_summary, Tags, test_scheme, test_vids, concepts):
    shot_index = []  # 所有视频的打乱后的索引
    for vid, clip_list in test_scheme:
        shot_index += clip_list
    ...
    for i in range(len(test_vids)):
        ...
        # 反向映射
        video_index = shot_index[pos : pos + vlength]
        reverse_ind = reverse_map(video_index)
        predictions = predictions[reverse_ind]
        pos += vlength
    ...
    with tf.Graph().as_default():
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                logits, attention = transformer(features, labels, positions, scores_src, scores_tgt, txt_emb, img_emb,
        ...
        test_scheme, test_vids = test_scheme_build_shuffle(data_test, hp.seq_len)
        ...
        for step in range(hp.maxstep):
            ...
            if step % epoch_step == 0 or (step + 1) == hp.maxstep:
                ...
                for test_step in range(max_test_step):
                    features_b, labels_b, positions_b, scores_b = get_batch_test_shuffle(data_test, test_scheme, test_step, hp.gpu_num, hp.bc, hp.seq_len)
                    scores_src_b = np.hstack((scores_b, np.ones((hp.gpu_num * hp.bc, CONCEPT_NUM))))  # encoder中开放所有concept节点
                    scores_tgt_b = scores_b
                    ...
                    atte_batch = atte_batch.reshape((hp.num_blocks, hp.num_heads, hp.bc, 173, 173))
                    atte_batch = np.transpose(atte_batch, (2, 0, 1, 3, 4))
                    atte_all.append(atte_batch)
                atte_all = np.array(atte_all).reshape((-1, hp.num_blocks, hp.num_heads, 173, 173))
                return atte_all
                ...
                # p, r, f = evaluation(pred_scores, query_summary, Tags, test_scheme, test_vids, concepts)
...
def main(self):
    ...
    attention = run_testing(data_train, data_test, queries, query_summary, Tags, concepts, concept_embedding,
                     CKPT_MODEL_PATH)
    np.save(r'/data/linkang/model_HL_v4/attention_0_0/attention.npy', attention)
    print('Done !')

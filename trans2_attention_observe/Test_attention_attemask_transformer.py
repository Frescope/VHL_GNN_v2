...
def segment_embedding(inputs, hp, scope='segment_embedding'):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        clip_embs= tf.concat(clip_embs, axis=0)  # seq_len*D_model
        clip_embs = tf.expand_dims(clip_embs, 0)  # 1*seq_len*D_model
        ...
        clip_nodes, concept_nodes = tf.split(inputs, [clip_len, concept_len], 1)  # 沿seq_len切开
        clip_nodes += clip_embs
        concept_nodes += concept_embs
        outputs = tf.concat([clip_nodes, concept_nodes], axis=1)
        return outputs

def mask(inputs, mask_meta, key_masks=None, type=None):
    padding_num = -2 ** 32 + 1
    ...
    v_num, c_num, m_flag = mask_meta
    if m_flag:
        atte_mask = np.zeros((1, v_num+c_num, v_num+c_num))
        atte_mask[0, -c_num:, :v_num] = 1
        atte_mask = tf.cast(tf.convert_to_tensor(atte_mask), dtype=tf.float32)
        # atte_mask = tf.tile(atte_mask, [tf.shape(outputs)[0], 1, 1])
        outputs = outputs + atte_mask * padding_num
    return outputs

def scaled_dot_product_attention(Q, K, V, key_masks, mask_meta,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        # key masking
        outputs = mask(outputs, mask_meta, key_masks=key_masks, type="key")

        # causality or future blinding masking
        if causality:
            outputs = mask(outputs, mask_meta, type="future")
...
def multihead_attention(queries, keys, values, key_masks, mask_meta,
                        num_heads=8,
                        dropout_rate=0,
                        training=True,
                        causality=False,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        # Attention
        outputs, attention = scaled_dot_product_attention(Q_, K_, V_, key_masks, mask_meta, causality, dropout_rate, training)
...
def encoder(input_nodes, src_masks, mask_meta, drop_out, training, hp):
...
def transformer(features, positions, scores_src, txt_emb, img_emb, drop_out, training, hp, c_num, s_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        mask_meta = [hp.seq_len, c_num, hp.mask]

        # encoding & decoding
        memory, attention_outputs = encoder(input_nodes, src_masks, mask_meta, drop_out, training, hp)
        ...
        return concept_logits, summary_logits ,attention_outputs
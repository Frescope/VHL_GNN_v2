...
import h5py
import pickle
from trans2_attention_observe.Test_attention_conpred_transformer import transformer
...
if hp.server == 0:
    ...
    # CKPT_MODEL_PATH = r'/data/linkang/model_HL_v4/attention_conpred_75s/S12005-E245-L2.281410-F0.618'
    # CKPT_MODEL_PATH = r'/data/linkang/model_HL_v4/attention_conpred_35s/S1320-E15-L3.434120-F0.518'
    CKPT_MODEL_PATH = r'/data/linkang/model_HL_v4/attention_conpred_25s/S10720-E80-L2.191589-F0.605'
...
def tower_loss_2stages(concept_logits, concept_labels, concept_preds, seq_logits, seq_labels, hp):
    # concept_logits & concept_labels: bc*seq_len*48
    # concept_preds: bc*48*48
    ...
    with tf.Graph().as_default():
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                concept_logits, concept_preds, summary_logits, attention = transformer(features, positions, scores_src, txt_emb,
                                                                            img_emb,
                ...
                loss, loss_ob = tower_loss_2stages(concept_logits, concept_labels, concept_preds,
...
def main(self):
    ...
    attention = run_testing(data_train, data_test, queries, query_summary, Tags, concepts, concept_embedding,
                            CKPT_MODEL_PATH)
    np.save(r'/data/linkang/model_HL_v4/attention_conpred_25s/attention_conpred_25s.npy', attention)
    print('Done !')

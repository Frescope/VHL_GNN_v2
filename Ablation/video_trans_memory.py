# 基于video_trans_base，concept部分输入的变量可以训练，可以使用glove特征做初始化
...
import scipy.io
import pickle
from transformer_memory import transformer
...
class Path:
    ...
    parser.add_argument('--lr_noam', default=10e-5, type=float)
    ...
    parser.add_argument('--concept_init', default='text', type=str)
...
def run_training(data_train, data_test, queries, query_summary, Tags, concepts, concept_embeeding, model_save_dir, test_mode):
    ...
    with tf.Graph().as_default():
        ...
        # concept node processing
        txt_embs = []
        for c in concepts:
            txt_embs.append(concept_embeeding[c]['txt'])
        txt_embs = np.array(txt_embs).reshape([1, CONCEPT_NUM, D_TXT_EMB])
        if hp.concept_init == 'text':
            concept_init = tf.constant_initializer(txt_embs)
        elif hp.concept_init == 'random':
            concept_init = tf.truncated_normal_initializer(stddev=0.01)
        concept_nodes_seq = tf.get_variable(name='concept_nodes',
                                        shape=(1, CONCEPT_NUM, D_TXT_EMB),
                                        initializer=concept_init,
                                        dtype=tf.float32,
                                        trainable=True)
        concept_nodes = tf.tile(concept_nodes_seq, [hp.bc, 1, 1])
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                pred_s1_logits = transformer(features, concept_nodes, positions, scores_src, dropout_holder, training_holder, hp, D_C_OUTPUT)
        ...
        for step in range(hp.maxstep):
            ...
            observe = sess.run([train_op] +
                               loss_list +
                               pred_s1_logits_list +
                               [concept_nodes_seq, concept_nodes],

# 用于video_trans_base的基础encoder，没有concept部分
# 没有全局嵌入
...
def transformer(features, positions, scores_src, drop_out, training, hp, concept_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        # encoding & decoding
        memory = encoder(visual_nodes, src_masks, drop_out, training, hp)

        prediction_branch = tf.layers.dense(memory, 1024, use_bias=True, activation=tf.nn.relu)

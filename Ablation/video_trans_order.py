# 基于video_trans_base，顺序输入序列
# 正负样本分别计算loss，分别除以正负样本的数量以平衡权重
# 没有多个branch的输出以及对应的loss
# 没有全局嵌入
# 使用glove文本特征
...
import scipy.io
import pickle
from transformer_order import transformer
...
def train_scheme_build(data_train, concepts, query_summary, hp):
    ...
    for vid in data_train:
        ...
        for i in range(vlength - hp.seq_len + 1):
            clip_list = list(range(i, i + hp.seq_len))
...
def get_batch_train(data_train, train_scheme, step, hp):
    ...
    return features, positions, scores, s1_labels
...
def tower_loss_balance(pred_s1_logits, pred_s1_labels, hp):
    # pred_s1_logits & pred_s1_labels: bc*seq_len*48
    # 分别计算正例和负例的CE-loss，加权求和
    ...
    pos_mask = s1_labels_bin  # 正样本标签
    pos_logits = tf.clip_by_value(pos_mask * pred_s1_logits, 1e-6, 0.999999)  # 正样本输出保留，负样本全部为零
    pos_loss = - pos_mask * tf.log(pos_logits)
    pos_loss = tf.reduce_sum(pos_loss, axis=1) / (tf.reduce_sum(pos_mask, axis=1) + 1e-6)  # 除以每一行的正样本数
    pos_loss = tf.reduce_mean(pos_loss)

    # for negative samples
    neg_mask = 1 - s1_labels_bin  # 负样本标签，负样本为1
    neg_logits = tf.clip_by_value(neg_mask * pred_s1_logits, 1e-6, 0.999999)  # 负样本输出保留，正样本全部为零
    neg_loss = - neg_mask * tf.log(neg_logits)
    neg_loss = tf.reduce_sum(neg_loss, axis=1) / (tf.reduce_sum(neg_mask, axis=1) + 1e-6)
    neg_loss = tf.reduce_mean(neg_loss)

    s1_loss = pos_loss + neg_loss

    return s1_loss, [pos_loss, neg_loss]
    ...
    with tf.Graph().as_default():
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                pred_s1_logits = transformer(features, txt_emb, positions, scores_src, dropout_holder, training_holder, hp, D_C_OUTPUT)
                pred_s1_logits_list.append(pred_s1_logits)

                  # 训练时每个序列只针对一个query预测summary
                loss, loss_ob = tower_loss_balance(pred_s1_logits, pred_s1_labels, hp)
        ...
        for step in range(hp.maxstep):
            features_b, positions_b, scores_b, s1_labels_b = get_batch_train(data_train, train_scheme, step, hp)
            ...
            if step % epoch_step == 0 or (step + 1) == hp.maxstep:
                ...
                for test_step in range(max_test_step):
                    features_b, positions_b, scores_b = get_batch_test(data_test, test_scheme, test_step, hp)
                    scores_src_b = np.hstack((scores_b, np.ones((hp.gpu_num * hp.bc, CONCEPT_NUM))))  # encoder中开放所有concept节点

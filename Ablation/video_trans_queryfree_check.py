# 基于video_trans_base，去掉concept部分
...
import scipy.io
import pickle
from transformer_queryfree import transformer
...
def run_testing(data_train, data_test, queries, query_summary, Tags, concepts, concept_embeeding, model_path):
    with tf.Graph().as_default():
        ...
        scores_src_holder = tf.placeholder(tf.float32, shape=(hp.bc * hp.gpu_num, hp.seq_len))
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                pred_s1_logits = transformer(features, positions, scores_src, dropout_holder, training_holder, hp, D_C_OUTPUT)

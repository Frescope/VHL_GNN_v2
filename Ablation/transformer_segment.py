# 输入segment-embedding，并可添加position-encoding
...
def segment_embedding(inputs, hp, scope='segment_embedding'):
    ...
    concept_len = inputs.get_shape().as_list()[1] - hp.seq_len
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        clip_nodes, concept_nodes = tf.split(inputs, [hp.seq_len, concept_len], 1)  # 沿seq_len切开
...
def transformer(features, feature_positions, segment_embs, segment_positions, scores_src, drop_out, training, hp, concept_num):
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        input_nodes = tf.concat([visual_nodes, segment_nodes], axis=1)

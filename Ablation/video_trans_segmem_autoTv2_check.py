...
def evaluation_autothresh_test(pred_s1_lists, query_summary, Tags, test_vids, concepts, rank_num):
...
def run_testing(data_train, data_test, queries, query_summary, Tags, concepts, concept_embedding, segment_dict, model_path, rank_num):
    ...
    for kfold in range(4):
        ...
        for i in range(hp.repeat):
            ...
            for i in range(len(models_to_restore)):
                ...
                rank_num = math.floor(float(model_path.split('-')[-2].split('T')[-1]) * len(data[str((kfold + 2) % 4 + 1)]['feature']))

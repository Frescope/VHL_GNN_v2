...
import scipy.io
import pickle
from transformer_madvance import transformer
...
class Path:
    ...
    parser.add_argument('--msd', default='video_trans_madvance', type=str)
...
def run_testing(data_train, data_test, queries, query_summary, Tags, concepts, concept_embeeding, model_path):
    with tf.Graph().as_default():
        ...
        scores_src_holder = tf.placeholder(tf.float32, shape=(hp.bc * hp.gpu_num, hp.seq_len + hp.memory_num))
        ...
        memory_nodes_seq = tf.get_variable(name='concept_nodes',
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                pred_s1_logits = transformer(features, memory_nodes, positions, scores_src, dropout_holder, training_holder, hp, D_C_OUTPUT)
        ...
        for step in range(hp.maxstep):
            ...
            scores_src_b = np.hstack((scores_b, np.ones((hp.gpu_num * hp.bc, hp.memory_num))))  # encoder中开放所有concept节点

# video_trans_segment，基于video_trans_base
# 输入segment-embedding在序列后半部分，指定采样率（均匀分布），聚合方式，并添加segment-position-encoding
...
import scipy.io
import pickle
from transformer_segment import transformer
...
class Path:
    ...
    # segment-embedding参数
    parser.add_argument('--segment_num', default=100, type=int)  # segment节点数量
...
def run_training(data_train, data_test, queries, query_summary, Tags, concepts, concept_embeeding, segment_dict, model_save_dir, test_mode):
    ...
    with tf.Graph().as_default():
        ...
        scores_src_holder = tf.placeholder(tf.float32, shape=(hp.bc * hp.gpu_num, hp.seq_len + hp.segment_num))
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                pred_s1_logits = transformer(features, positions, segment_embs, segment_poses, scores_src, dropout_holder, training_holder, hp, D_C_OUTPUT)
        ...
        for step in range(hp.maxstep):
            ...
            scores_src_b = np.hstack((scores_b, np.ones((hp.gpu_num * hp.bc, hp.segment_num))))  # encoder中开放所有concept节点

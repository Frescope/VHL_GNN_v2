...
def evaluation_autothresh_test(concept_lists, summary_lists, query_summary, Tags, test_vids, concepts, queries, table):
    ...
    for i in range(len(test_vids)):
        ...
        for query in summary:
            ...
            threshold = (table[c1] + table[c2]) / 2  # 取两个concept的门限均值作为这一query所对应的门限
...
def run_testing(data_train, data_test, queries, query_summary, Tags, concepts, concept_embeeding, model_path, table):
    with tf.Graph().as_default():
        ...
        for step in range(hp.maxstep):
            ...
            if step % epoch_step == 0 or (step + 1) == hp.maxstep:
                ...
                p, r, f = evaluation_autothresh_test(concept_lists, summary_lists, query_summary, Tags, test_vids, concepts, queries, table)
    ...
    for kfold in range(4):
        ...
        for i in range(hp.repeat):
            ...
            for i in range(len(models_to_restore)):
                ...
                with open(model_path+'_table.json', 'r') as file:

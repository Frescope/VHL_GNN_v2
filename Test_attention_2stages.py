...
import h5py
import pickle
from Test_attention_2stages_transformer import transformer
...
if hp.server == 0:
    ...
    CKPT_MODEL_PATH = r'/data/linkang/model_HL_v4/attention_noisy_25s/S9540-E90-L2.670906-F0.381'
...
def train_scheme_build_2stages_ob(data_train, concepts, query_summary, hp):
    # 用于两阶段预测的序列构建
    info_dict = {}
    ...
    for vid in data_train:
        ...
        for query in query_summary[vid]:
            ...
            qs_pos_list = info_dict[vid][query]['qs_pos']
            k = math.ceil(max(len(c1_pos_list), len(c2_pos_list)) / cp_num)  # 取正例的循环次数，不足时从头循环
            qs_part = random.sample(qs_pos_list, qs_num)  # 随机取若干qs正例
            for i in range(k):  # 生成k个输入序列
                ...
                # 按照类型分组
                clip_type_list = []  # 每个分段的编号及其类型：qs正样本、1正样本、c2正样本、负样本
                pos_num = len(clip_list)  # 正样本数量
                for clip in clip_list:
                    if clip in qs_pos_list:
                        clip_type_list.append((clip, 0))
                    elif clip in c1_pos_list:
                        clip_type_list.append((clip, 1))
                    elif clip in c2_pos_list:
                        clip_type_list.append((clip, 2))
                    else:
                        clip_type_list.append((clip, 3))
                        pos_num -= 1
                clip_type_list.sort(key=lambda x: x[1])  # 按照类型分组排列

                train_scheme.append((vid, query, clip_list, pos_num))
    return train_scheme

def get_batch_train_2stages_ob(data_train, train_scheme, queries, concepts, step, gpu_num, bc, seq_len):
    ...
    meta_list = []  # 记录每个输入序列的meta数据
    for i in range(batch_num):
        pos = (step * batch_num + i) % len(train_scheme)
        vid, query, clip_list, pos_num = train_scheme[pos]
        meta_list.append((vid, query, pos_num))
        f_temp = []
        for clip in clip_list:
            f_temp.append(data_train[vid]['feature'][clip, 3, :])
    ...
    return features, positions, scores, concept_labels, summary_labels, qc_indexes, meta_list
...
def get_batch_test(data_test, test_scheme, step, gpu_num, bc, seq_len):
    ...
    for i in range(batch_num):
        ...
        feature = data_test[str(vid)]['feature'][seq_start:seq_end, 3, :]
    ...
    with tf.Graph().as_default():
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                concept_logits, summary_logits, attention = transformer(features, positions, scores_src, txt_emb, img_emb,
        ...
        train_scheme = train_scheme_build_2stages_ob(data_train, concepts, query_summary, hp)
        ...
        for step in range(epoch_step):
            features_b, positions_b, scores_b, concept_labels_b, summary_labels_b, query_indexes_b, meta_b = \
                get_batch_train_2stages_ob(data_train, train_scheme, queries, concepts, step, hp.gpu_num, hp.bc, hp.seq_len)
            scores_src_b = np.hstack((scores_b, np.ones((hp.gpu_num * hp.bc, CONCEPT_NUM))))  # encoder中开放所有concept节点
            atte_batch = sess.run(attention_list,
            ...
            atte_batch = atte_batch.reshape(
                (hp.num_blocks, hp.num_heads, hp.bc, hp.seq_len + CONCEPT_NUM, hp.seq_len + CONCEPT_NUM))
            atte_batch = np.transpose(atte_batch, (2, 0, 1, 3, 4))
            atte_all.append(atte_batch)
            meta_all += meta_b
        atte_all = np.array(atte_all).reshape(
            (-1, hp.num_blocks, hp.num_heads, hp.seq_len + CONCEPT_NUM, hp.seq_len + CONCEPT_NUM))

        atte_outputs = []
        for i in range(len(meta_all)):
                atte_outputs.append((meta_all[i], atte_all[i]))
        return atte_outputs
...
def main(self):
    ...
    attention_outputs = run_training(data_train, data_valid, queries, query_summary, Tags, concepts, concept_embedding, 0)
    with open(r'/data/linkang/model_HL_v4/attention_output.pkl', 'wb') as file:
        pickle.dump(attention_outputs, file)
    print('Done !')

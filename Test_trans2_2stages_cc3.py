# 基于trans2_2stages构建，大乱序+小顺序的序列构建方式
# 每个序列中从q、c1、c2中各选择一定比例的正例，构成正例集合；
# 从每个正例的左右邻域内选择连续的一组片段构成一个顺序的子片段，然后将所有子片段拼接成一个乱序的输入序列。
...
class Path:
    ...
    parser.add_argument('--seq_len',default=50,type=int)  # 序列整体长度
    parser.add_argument('--subseq_len', default=5, type=int)  # 每个连续子片段的长度
    ...
    parser.add_argument('--qs_pr', default=0.04, type=float)  # query-summary positive ratio
...
def seq_build(vlength, pos_list, offset, hp):
    # 根据采样点构建完整的输入序列信息
    clip_list = []
    if pos_list[0] + offset[0] >= 0 and pos_list[-1] + offset[-1] <= vlength:
        for pos in pos_list:
            clip_list += list(range(pos + offset[0], pos + offset[-1]))
    else:
        for pos in pos_list:
            start = max(0, offset[0] + pos)
            end = min(start + hp.subseq_len, vlength)
            clip_list += list(range(start, end))
    clip_list = list(set(clip_list))
    while len(clip_list) < hp.seq_len:  # padding
...
def train_scheme_build_2stages(data_train, concepts, query_summary, hp):
    # 用于两阶段预测的序列构建
    info_dict = {}
    offset = (-math.floor(hp.subseq_len / 2), -math.floor(hp.subseq_len / 2) + hp.subseq_len)
    ...
    # 构建序列
    train_scheme = []
    subseq_num = int(hp.seq_len / hp.subseq_len)  # 每个子片段中保证至少一个正例
    qs_num = math.ceil(hp.seq_len * hp.qs_pr)
    c1_num = max(0, math.ceil((subseq_num - qs_num) / 2))  # 从c1中取的正例数目
    c2_num = max(0, subseq_num - qs_num - c1_num)  # 从c2中取的正例数目
    for vid in data_train:
        ...
        for query in query_summary[vid]:
            ...
            k = math.ceil(max(len(c1_pos_list), len(c2_pos_list)) / min(c1_num, c2_num))  # 取正例的循环次数，不足时从头循环
            qs_part = random.sample(qs_pos_list, qs_num)  # 随机取若干qs正例
            for i in range(k):  # 生成k个输入序列
                # 取c1相关的序列
                c1_pos_ind = i * c1_num % len(c1_pos_list)  # 从正例集合中取c1_nun个正例
                c1_part = c1_pos_list[c1_pos_ind: c1_pos_ind + c1_num]
                c1_part += c1_pos_list[0 : c1_num - len(c1_part)]  # 负例不足时做padding，一般不起作用
                # 取c2相关的序列
                c2_pos_ind = i * c2_num % len(c2_pos_list)  # 从正例集合中取c2_nun个正例
                c2_part = c2_pos_list[c2_pos_ind: c2_pos_ind + c2_num]
                c2_part += c2_pos_list[0: c2_num - len(c2_part)]  # 负例不足时做padding，一般不起作用
                pos_list = list(set(qs_part + c1_part + c2_part))  # 构建正例序列，作为后面扩展成完整序列的基础
                pos_list_all = list(set(qs_pos_list + c1_pos_list + c2_pos_list))  # 所有相关的正例，用于做padding
                while len(pos_list) < subseq_num:
                    pad_clip = random.sample(pos_list_all, 1)[0]
                    if pad_clip not in pos_list:
                        pos_list.append(pad_clip)
                pos_list.sort()
                clip_list = seq_build(vlength, pos_list, offset, hp)
...
def main(self):
    ...
    for kfold in range(4):
        ...
        logging.info('Sub-Sequence Length: ' + str(hp.subseq_len))

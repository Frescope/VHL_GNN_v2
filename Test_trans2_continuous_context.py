...
class Path:
    ...
    parser.add_argument('--bc',default=40,type=int)
...
def get_batch_train_cc(data_train, train_scheme, step, hp):
    ...
    return features, labels, positions, sample_poses, scores, concept_id
...
def tower_loss_cc(logits,labels, concept_id):
    # logits: bc*48, labels: bc
    # 取logits中特定concept对应的行，与labels计算NCE-Loss
    logits = tf.reshape(logits[:, concept_id], shape=(-1,))
    labels = tf.reshape(labels, shape=(-1,))
    labels_binary = tf.cast(tf.cast(labels, dtype=tf.bool), dtype=tf.float32)  # 转化为0-1形式，浮点数
    nce_pos = tf.reduce_sum(tf.exp(labels_binary * logits))  # 分子
    nce_pos -= tf.reduce_sum((1 - labels_binary))  # 减去负例（为零）取e后的值（为1）
    nce_all = tf.reduce_sum(tf.exp(logits))   # 分母
...
def run_training(data_train, data_test, queries, query_summary, Tags, concepts, concept_embeeding, model_save_dir, test_mode):
    ...
    with tf.Graph().as_default():
        ...
        labels_holder = tf.placeholder(tf.float32, shape=(hp.bc * hp.gpu_num, ))
        ...
        concept_id_holder = tf.placeholder(tf.int32, shape=())
        ...
        for gpu_index in range(hp.gpu_num):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                loss, loss_ob = tower_loss_cc(logits,labels,concept_id)
        ...
        for step in range(hp.maxstep):
            features_b, labels_b, positions_b, sample_poses_b, scores_b, concept_id_b = get_batch_train_cc(data_train, train_scheme, step, hp)
            ...
            observe = sess.run([train_op] + loss_list + logits_list + loss_ob_list,
                               feed_dict={features_holder: features_b,
                                          ...
                                          concept_id_holder: concept_id_b,

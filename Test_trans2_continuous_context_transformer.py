...
def transformer(features, labels, positions, sample_poses, scores_src, img_emb, drop_out, training, hp):
    # 对前端和末端填充的部分position_encoding填零
    # positions为序列中所有节点的绝对位置，sample_pos为取样点在序列中的相对位置
    with tf.variable_scope("transformer", reuse=tf.AUTO_REUSE):
        ...
        logits = tf.layers.dense(logits, c_num, use_bias=True, activation=None)  # bc*seqlen*48
